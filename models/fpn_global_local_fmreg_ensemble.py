...
class fpn_module_global(nn.Module):
    def __init__(self, numClass):
        ...
        self._up_kwargs = {'mode': 'bilinear', 'align_corners': True}#增加'align_corners': True
        # Top layer
...
#     def forward(self, c2_g, c2_l):
#         batch_size, _, height, width = c2_g.size()

#         # Flatten the spatial dimensions and apply linear layer
#         Q = self.linear_q(c2_g.view(batch_size, self.d_model, -1)).view(batch_size, self.d_model, height, width)
#         K = self.linear_k(c2_l.view(batch_size, self.d_model, -1)).view(batch_size, self.d_model, height, width)
#         V = self.linear_v(c2_l.view(batch_size, self.d_model, -1)).view(batch_size, self.d_model, height, width)

#         # Reshape to fit the expected input shape of MultiheadAttention: (seq_len, batch_size, embedding_dim)
...
#         # Reshape attn_output to original shape: (batch_size, num_channels, height, width)
#         attn_output = attn_output.permute(1, 2, 0).view(batch_size, self.d_model, height, width)
...
#     def forward(self, c2_g_list, c2_l):
#         batch_size, _, height_l, width_l = c2_l.size()
#         outputs = []
#         for c2_g in c2_g_list:
#             _, _, height_g, width_g = c2_g.size()
#             Q = self.linear_q(c2_g.view(batch_size, self.d_model, -1)).view(batch_size, self.d_model, height_g, width_g)
#             K = self.linear_k(c2_l.view(batch_size, self.d_model, -1)).view(batch_size, self.d_model, height_l, width_l)
#             V = self.linear_v(c2_l.view(batch_size, self.d_model, -1)).view(batch_size, self.d_model, height_l, width_l)
...
#             attn_output = attn_output.permute(1, 2, 0).view(batch_size, self.d_model, height_g, width_g)
...
#     def forward(self, c2_g, c2_l):
#         batch_size, c_dim, _, _ = c2_l.size()
#         outputs = []
#         _, _, height_g, width_g = c2_g.size()
#         # self.linear_tran = nn.Linear(c, d_model)
#         # self.linear_tran1 = nn.Linear(d_mode
        
        
#         upsample = nn.Upsample(size=(height_g, width_g), mode='bilinear', align_corners=True).to(c2_l.device)
#         c2_l_up = upsample(c2_l)

#         Q = self.linear_q(c2_g.view(batch_size, -1, self.d_model)).permute(0, 2, 1).view(batch_size, self.d_model, height_g, width_g)
#         K = self.linear_k(c2_l_up.view(batch_size, -1, self.d_model)).permute(0, 2, 1).view(batch_size, self.d_model, height_g, width_g)
#         V = self.linear_v(c2_l_up.view(batch_size, -1, self.d_model)).permute(0, 2, 1).view(batch_size, self.d_model, height_g, width_g)
...
class AttentionModel(nn.Module):
    ...
    def forward(self, c2_g, c2_l):
        ...
        Q = self.linear_q(c2_g.view(batch_size, self.d_model, -1).permute(0, 2, 1))
        K = self.linear_k(c2_l_up.view(batch_size, self.d_model, -1).permute(0, 2, 1))
        V = self.linear_v(c2_l_up.view(batch_size, self.d_model, -1).permute(0, 2, 1))
...
class CrossAttention(nn.Module):
    ...
    def forward(self, c_g, c_l):
        # print(c_g.shape)
        b, c, h, w = c_g.shape

        c_g = c_g.view(b, c, -1).permute(0, 2, 1)  # now of shape (b, h*w, c)
        c_l = c_l.view(b, c, -1).permute(0, 2, 1)  # now of shape (b, h*w, c)
        ...
        attn_output, _ = self.cross_attention1(Q, K, V)
        attn_output = attn_output.permute(1, 2, 0).view(b, self.d_model1, h, w)
...
#         super(FlexibleAttentionModel, self).__init__()
#         self.input_linear = nn.ModuleDict({
#             '256': nn.Linear(256, 512),
#             '512': nn.Linear(512, 512),
#             '1024': nn.Linear(1024, 512),
#             '2048': nn.Linear(2048, 512),
#         })
#         self.model = AttentionModel(d_model=512, num_heads=8)
#         self.output_linear = nn.ModuleDict({
#             '256': nn.Linear(512, 256),
#             '512': nn.Linear(512, 512),
#             '1024': nn.Linear(512, 1024),
#             '2048': nn.Linear(512, 2048),
#         })

#     def forward(self, input1, input2, size):
#       size = str(size)
#       input1 = self.input_linear[size](input1)
#       input2 = self.input_linear[size](input2)
#       output = self.model(input1, input2, size)  # 传递size参数给self.model的forward方法
#       if len(output) > 0:
#           return [self.output_linear[size](output[0])]
#       else:
#           return []
...
class fpn(nn.Module):
    ...
    def _calculate_entropy(self, tensor):  #增加
        '''
        Calculate the entropy of a tensor along the channel dimension
        tensor: tensor of shape (batch_size, C, H, W)
        '''
        # apply softmax on the channel dimension
        tensor = tensor.softmax(dim=1)
        tensor += torch.finfo(float).eps

        entropy_maps = []
        for i in range(tensor.shape[0]):  # iterate over the batch dimension
            entropy = -torch.sum(tensor[i] * torch.log2(tensor[i]), dim=0)  # calculate entropy along the channel dimension
            # print(entropy.shape)
    ...
    def _merge_local(self, f_local, merge, f_global, top_lefts, oped, ratio, template):
        ...
        _, c, H, W = f_global.size() # match global feature size
        if merge is None:
            merge = f_global.clone()  # Make a copy of f_global instead of initializing merge with zeros.
        h, w = int(np.round(H * ratio[0])), int(np.round(W * ratio[1]))
        for i in range(b):
            index = oped[0] + i
            top, left = int(np.round(H * top_lefts[index][0])), int(np.round(W * top_lefts[index][1]))
    ...
    def forward(self, image_global, patches, top_lefts, ratio, mode=1, global_model=None, n_patch=None):
        if mode == 1:
            ...
            # model2 = AttentionModel(d_model=256, num_heads=8).to(device)
            # model3 = AttentionModel(d_model=512, num_heads=8).to(device)
            # model4 = AttentionModel(d_model=1024, num_heads=8).to(device)
            # model5 = AttentionModel(d_model=2048, num_heads=8).to(device)
            # self.merge_c2_g = model2(self._crop_global(self.c2_g, top_lefts, ratio), c2_l)
            # self.merge_c3_g = model3(self._crop_global(self.c3_g, top_lefts, ratio), c3_l)
            # self.merge_c4_g = model4(self._crop_global(self.c4_g, top_lefts, ratio), c4_l)
            # self.merge_c5_g = model5(self._crop_global(self.c5_g, top_lefts, ratio), c5_l)
            
            model1 = AttentionModel(d_model=256, num_heads=8).to(device)

            self.merge_c2_g = model1(self._crop_global(self.c2_g, top_lefts, ratio)[0], c2_l)

            model2 = AttentionModel(d_model=512, num_heads=8).to(device)

            self.merge_c3_g = model2(self._crop_global(self.c3_g, top_lefts, ratio)[0], c3_l)

            model3 = AttentionModel(d_model=1024, num_heads=8).to(device)

            self.merge_c4_g = model3(self._crop_global(self.c4_g, top_lefts, ratio)[0], c4_l)

            model4 = AttentionModel(d_model=2048, num_heads=8).to(device)

            self.merge_c5_g = model4(self._crop_global(self.c5_g, top_lefts, ratio)[0], c5_l)
            
            # self.merge_c2_g = model2(self._crop_global(self.c2_g, top_lefts, ratio)[0], c2_l, 256)
            # self.merge_c3_g = model2(self._crop_global(self.c3_g, top_lefts, ratio)[0], c3_l, 512)
            # self.merge_c4_g = model2(self._crop_global(self.c4_g, top_lefts, ratio)[0], c4_l, 1024)
            # self.merge_c5_g = model2(self._crop_global(self.c5_g, top_lefts, ratio)[0], c5_l, 2048)
            ...
            # print(c2_g.shape)
            # print(self.c2_l.shape)
            
            
            device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
            
#             model5 = CrossAttention(d_model=256, num_heads=1).to(device)

#             self.aug_c2_g = model5(c2_g, self.c2_l)
            
            model6 = CrossAttention(d_model=512, num_heads=4).to(device)

            self.aug_c3_g = model6(c3_g, self.c3_l)

            model7 = CrossAttention(d_model=1024, num_heads=8).to(device)

            self.aug_c4_g = model7(c4_g, self.c4_l)

            model8 = CrossAttention(d_model=2048, num_heads=8).to(device)

            self.aug_c5_g = model8(c5_g, self.c5_l)
            

            

            
            output_g, ps0_g, ps1_g, ps2_g, ps3_g = self.fpn_global.forward(c2_g, self.aug_c3_g, self.aug_c4_g, self.aug_c5_g, c2_ext=self.c2_l, c3_ext=self.c3_l, c4_ext=self.c4_l, c5_ext=self.c5_l, ps0_ext=self.ps0_l, ps1_ext=self.ps1_l, ps2_ext=self.ps2_l)
            # imsize = image_global.size()[2:]
            # output_g = F.interpolate(output_g, imsize, mode='nearest')
            self.clear_cache()
